...
void ui::render() {
    ...
    ImGui::SetNextWindowSize(ImVec2(600, 400)); // Increased window size
...
void renderMainPage() {
    ...
    // Click Randomization
    ImGui::Spacing();
    ImGui::Text("Click Randomization:");
    static int randomPositionRange = 0;
    static int randomDelayJitter = 0;
    ImGui::SliderInt("Position Variability (pixels)", &randomPositionRange, 0, 50, "%d px");
    ImGui::SliderInt("Delay Jitter (ms)", &randomDelayJitter, 0, 200, "%d ms");

    // Store updated values in globals
    globals.randomPositionRange = randomPositionRange;
    globals.randomDelayJitter = randomDelayJitter;
}
